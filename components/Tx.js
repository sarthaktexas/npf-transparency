...
export default function Transaction({ vendorImage, vendorName, txName, price, modalTitle, modalDescription, modalExitMessage }) {
    return (
        <div className={`relative p-5 rounded-xl flex flex-col space-y-5 md:space-y-0 md:grid md:grid-cols-6 md:gap-5 ${price?.substring(0,1) === '-' ? 'bg-red-100 dark:bg-red-800 dark:bg-opacity-40' : 'bg-green-100 dark:bg-green-800 dark:bg-opacity-50'} md:h-24 md:items-center justify-center content-center`}>
